    ...
    <section class="container-fluid">
        <div class="row">
            ...
            <div class="form-group col-md-3" style="text-align: center">
                ...
                <!-- onchange="consultar_documento(this.value)" -->
                <input type="tel" class="form-control" id="cliente_numero"  placeholder="Ingresar DNI ó RUC..." onkeypress="return valida(event)">
            </div>
        </div>
        <br>
        <div class="row" id="div_documento_dni">
            ...
            <div class="col-lg-6">
                <label for="">Seleccionar Horario <label for="" class="text-danger">(* No necesario en Docente)</label></label>
                <select name="horario" id="horario" class="form-control">
                    <option value="0">-----------------------------</option>
                    <?php foreach ($horarios as $h){ ?>
                        <option value="<?= $h->id_horario ?>"><?= $h->descripcion .' ('. $h->aula_nombre .') '.$h->modalidad_nombre.'' .$h->ingreso.'/'.$h->salida  ?></option>

                    <?php } ?>
                </select>
            </div>
            <div class="col-lg-6">
                <label for="id_carrera">Carrera <label for="" class="text-danger">(* No necesario en Docente)</label></label>
                <select name="id_carrera" id="id_carrera" class="form-control">
                    <option value="0">-----------------------------</option>
                    <?php foreach ($carreras as $ca){ ?>
                        <option value="<?= $ca->id_carrera ?>"><?= $ca->carrera_descripcion  ?></option>

                    <?php } ?>
                </select>
            </div>
            <div class="col-lg-3">
                <label for="">Número de veces que postula</label>
                <input type="number" class="form-control" id="postulaciones">
            </div>
            <div class="col-lg-6">
                <label for="">Colegio de Procedencia</label>
                <input type="text" class="form-control" id="colegio" onkeyup="mayuscula(this.id)">
            </div>
            <div class="col-lg-12">
                <label >Nombre Apoderado </label>
                <input type="text" id="apoderado" class="form-control" onkeyup="mayuscula(this.id)">
            </div>
            <div class="col-lg-12">
                <label >Celular Apoderado </label>
                <input type="text" id="cel_apoderado" class="form-control" onkeyup="validar_numeros(this.id)">
            </div>
            <div class="col-lg-12">
                <label >Observaciones </label>
                <input type="text" id="observaciones" class="form-control" onkeyup="validar_numeros(this.id)">
            </div>
            <div class="form-group col-md-12">
                <button id="btn-agregar" class="btn btn-success" style="width: 100%; margin-top: 37px" onclick="guardar()"><i class="fa fa-plus"></i> Agregar Alumno</button>
            </div>
        </div>
        <!-- /.row -->
    </section>
