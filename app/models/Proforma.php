...
class Proforma
{
    ...
    public function guardar_proforma($model){
        try{
            $sql = 'insert into proformas (id_cliente, id_usuario, id_moneda, proforma_correlativo, proforma_total, proforma_fecha_generada, 
    ...
    public function guardar_detalle_proforma($model){
        try{
            $sql = 'insert into proforma_detalle (id_proforma, id_producto_precio, id_medida, proforma_detalle_precio, proforma_detalle_producto_cantidad, 
                    producto_proforma_total_selled, proforma_detalle_mm, proforma_detalle_fecha_registro, proforma_detalle_estado) values (?,?,?,?,?,?,?,?,?)';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([
                $model->id_proforma,
    ...
    public function listar_proforma(){
        try {
            $sql = 'select * from proformas p inner join clientes c on p.id_cliente = c.id_cliente';
            $stm = $this->pdo->prepare($sql);
            $stm->execute();
            $result = $stm->fetchAll();
        } catch (Exception $e){
    ...
    public function datos_proforma($id){
        try {
            $sql = 'select * from proforma_detalle pd inner join proformas p on pd.id_proforma = p.id_proforma inner join clientes c on p.id_cliente = c.id_cliente
                    where p.id_proforma = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            $result = $stm->fetch();
    ...
    public function listar_datos_pdf($id){
        try {
            $sql = 'select * from proformas p inner join proforma_detalle pd on p.id_proforma = pd.id_proforma inner join producto_precio pp on pd.id_producto_precio = pp.id_producto_precio
                    inner join producto p2 on pp.id_producto = p2.id_producto where p.id_proforma = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            $result = $stm->fetchAll();
        } catch (Exception $e){
    ...
    public function jalar_valor($id_productforsaleb){
        try{
            $sql = 'select * from producto p inner join producto_precio pp on p.id_producto = pp.id_producto where pp.id_producto_precio = ? limit 1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_productforsaleb]);
            $result = $stm->fetch();
        } catch (Exception $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            $result = [];
